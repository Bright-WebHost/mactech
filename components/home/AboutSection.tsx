...
export default function AboutSection() {
  return (
    <section style={{ background:'#f5f4f0', padding:'clamp(60px,8vw,80px) 0', overflow:'hidden', position:'relative', fontFamily:"'Barlow', sans-serif" }}>
      <motion.div
        initial={{ opacity:0, x:-40 }}
        animate={{ opacity:1, x:0 }}
        transition={{ duration:1, ease:'easeOut' }}
        style={{ position:'absolute', top:'50%', left:'-30px', transform:'translateY(-50%)', fontFamily:"'Barlow Condensed', sans-serif", fontSize:'clamp(80px,14vw,200px)', fontWeight:900, color:'rgba(226,0,16,0.05)', lineHeight:1, pointerEvents:'none', userSelect:'none', letterSpacing:'-4px', zIndex:0 }}>
        ABOUT
      </motion.div>
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'0 clamp(20px,5vw,6vw)', position:'relative', zIndex:1 }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'80px', alignItems:'center' }}>
          ...
          <div>
            ...
            <Reveal delay={0.1}>
              <h2 style={{ fontFamily:"'Barlow Condensed', sans-serif", fontSize:'clamp(30px,4vw,56px)', fontWeight:900, lineHeight:0.95, letterSpacing:'-1px', textTransform:'uppercase', color:'#1a1a1a', marginBottom:24 }}>
            ...
            <Reveal delay={0.2}>
              <div>
                <p style={{ fontSize:14, lineHeight:1.8, color:'#666', marginBottom:14 }}>
                ...
                <p style={{ fontSize:14, lineHeight:1.8, color:'#666', marginBottom:0 }}>
              ...
              {[
                ...
                <Reveal key={feat.title} delay={0.3 + i * 0.1}>
                  <motion.div whileHover={{ x:4 }}
                    style={{ borderLeft:'3px solid #E20010', padding:'12px 16px', background:'rgba(255,255,255,0.6)', cursor:'pointer' }}>
          ...
          <div style={{ position:'relative', height:'420px' }}>
            <motion.div initial={{ opacity:0, scale:0.8 }} animate={{ opacity:1, scale:[1,1.03,1] }}
              ...
              style={{ position:'absolute', top:-16, left:-16, width:'55%', height:'60%', background:'#111', zIndex:0 }}/>
            ...
            <motion.div initial={{ opacity:0, scale:0.8 }} animate={{ opacity:1, scale:[1,1.04,1] }}
              ...
              style={{ position:'absolute', bottom:-16, right:-16, width:'45%', height:'50%', background:'#E20010', zIndex:0 }}/>

            <motion.div initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }} transition={{ duration:0.8, delay:0.15 }}
              style={{ position:'absolute', top:24, left:24, right:24, bottom:24, zIndex:1, overflow:'hidden' }}>
            ...
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:[0,-8,0] }}
              ...
              style={{ position:'absolute', bottom:36, left:36, background:'#E20010', padding:'14px 20px', zIndex:2 }}>
              <div style={{ fontFamily:"'Barlow Condensed', sans-serif", fontSize:36, fontWeight:900, lineHeight:1, color:'#fff' }}>15+</div>
              <div style={{ fontSize:10, fontWeight:600, letterSpacing:'1.5px', textTransform:'uppercase', color:'rgba(255,255,255,0.85)', marginTop:4 }}>Years Experience</div>
            </motion.div>
          </div>
        </div>
      </div>
